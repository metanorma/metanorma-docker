...
Important requirment for `tag`: it must correlate with versioning of ["metanorma-cli", link="https://github.com/metanorma/metanorma-cli/releases"]
...
To ease the process, the docker image will setup everything for you so you don't
have to worry about anything. In a high level the image incldues are followings
...
* PlantUML with the Java subsystem
* LaTeXML `0.8.4` with Perl
* Ruby 2.5 with Metanorma and Metanorma CLI

If you are interested to see the full list of packages/software then please
checkout https://github.com/metanorma/metanorma-linux-setup/blob/master/ubuntu.sh[this script]
